...
export const getPluginConfig = async (
	...
	let userConfig: UserConfig | null | undefined = null;

	if (typeof plugin.config === "function")
		userConfig = (await plugin.config({}, env)) ?? undefined;

	if (userConfig) return (userConfig as ViteConfig).VitePluginNodeConfig;
...
export const createMiddleware = async (
	...
	return async (
		...
		if (app) await requestHandler({ app, server, req, res, next });
	};
};
